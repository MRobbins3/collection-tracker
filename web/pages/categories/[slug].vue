...
// Fallback: turn `set_number` into `Set number` when a schema author forgot
// to set `title`. Keeps the UI readable even before we audit every category.
      ...
      >
        ...
        <dl class="mt-3 space-y-3">
          <div v-for="a in attributeLabels" :key="a.key">
            <dt class="text-sm font-medium text-slate-900">{{ a.title }}</dt>
            <dd v-if="a.description" class="mt-0.5 text-sm text-slate-600">{{ a.description }}</dd>
          </div>
        </dl>
      </section>
